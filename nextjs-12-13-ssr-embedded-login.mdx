---
title: "Embedded Login Integration (Next.js)"
---

### What will we build?

In this quick start, we will add the Frontegg Embedded login box to your Next.js application.
...
> 
> You will need to configure the `Single Sign On URL` on your IDP to use `[your-application-url]/api/frontegg/saml/callback`, instead of the ACS URL that you have configured on the Frontegg portal. 
> 
> The configuration on the Frontegg portal can stay the same as configured per [Configuring SAML](/docs/configuring-saml#acs-url) guide.
