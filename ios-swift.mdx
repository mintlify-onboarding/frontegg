...
Prepare to delve into the realm of iOS app development with Swift and Frontegg. In this guide, we will guide you through the integration of Frontegg's capabilities into iOS applications.
...
#### Preparing Your Frontegg Workspace

Navigate to [Frontegg Portal Settings](https://portal.frontegg.com/development/settings), If you don't have an pplication yet,  
Follow the [Onboarding Toolkit](/docs/onboarding-toolkit) steps to initiate a new app. 

Copy the FronteggDomain for future steps from your [Frontegg Portal Domain](https://portal.frontegg.com/development/settings/domains)
...
[](#adding-frontegg-uikit-wrapper)  
   ...
   * Create FronteggController class that extends AbstractFronteggController from FronteggSwift  
   ...
   `  
   * Create a new ViewController and set FronteggController as a view custom class from the previous step  
   ![ViewController custom class](/images/README_custom-class.png)  
   * Mark FronteggController as **Storyboard Entry Point**  
   ![ViewController entry point](/images/README_entry-point.png)  
   * Setup SceneDelegate for Frontegg universal links:  
   ...
   `  
   * Access authenticated user via `FronteggApp.shared.auth`  
   Swift  
   `  
     //  
...
#### Configuring iOS-associated domain
...
> ### 📘
> 
> To use our API’s, go to Getting Started with Frontegg APIs to generate a vendor token.